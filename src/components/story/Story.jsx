...
function Story({ cardId, onClose }) {
    ...
    useEffect(() => {
        ...
        const intervalId = setInterval(() => {
            setCurrentImageIndex((prevIndex) => {
                const nextIndex = prevIndex + 1;
                return nextIndex < cardData.slides.length ? nextIndex : 0; // Loop back to the first slide
            });
            setProgress(0);
        }, 7000);
    ...
    const handleNextSlide = () => {
        if (!cardData || !cardData.slides || cardData.slides.length === 0) return;
        setCurrentImageIndex((prevIndex) => (prevIndex + 1) % cardData.slides.length); // Ensure the index loops back to 0 when reaching the end
    };

    const handlePrevSlide = () => {
        if (!cardData || !cardData.slides || cardData.slides.length === 0) return;
        setCurrentImageIndex((prevIndex) => (prevIndex - 1 + cardData.slides.length) % cardData.slides.length); // Ensure the index loops back to the last slide when reaching 0
    };
